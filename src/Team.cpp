...
void Team::initializeFormation(float windowWidth, float windowHeight, float startX) {
    ...
    if (startX < windowWidth / 2) {
        players.emplace_back(30, windowHeight / 2, 1, teamColor);
    } else {
        players.emplace_back(windowWidth - 30, windowHeight / 2, 1, teamColor);
    }
...
void Team::update(Ball& ball, float windowWidth, float windowHeight) {
    if (players.empty()) initializeFormation(windowWidth, windowHeight, startX);
    for (auto& player : players) player.update(windowWidth, windowHeight);
}

void Team::render(sf::RenderWindow& window) {
    for (auto& player : players) window.draw(player.getShape());
}
