...
void Game::loadFont() {
    ...
    instructionsText.setString("Controls: WASD/Arrows - Move Selected Player | Click - Select Player | Space - Pass/Shoot | R - Reset | P - Pause");
}
...
void Game::handleInput() {
    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
            ...
            // Select player from either team
    ...
    // Team A controls (WASD)
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) {
        teamA.moveSelectedPlayer(0, -2);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) {
        teamA.moveSelectedPlayer(0, 2);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
        teamA.moveSelectedPlayer(-2, 0);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::D)) {
        teamA.moveSelectedPlayer(2, 0);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
        teamA.shootWithSelected(ball);
    }
    
    // Team B controls (Arrow Keys)
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
        teamB.moveSelectedPlayer(0, -2);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) {
        teamB.moveSelectedPlayer(0, 2);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
        teamB.moveSelectedPlayer(-2, 0);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) {
        teamB.moveSelectedPlayer(2, 0);
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Return)) {
        teamB.shootWithSelected(ball);
    }
}
...
void Game::update(float deltaTime) {
    ...
    // Update time display (convert to minutes:seconds)
...
void Game::checkCollisions() {
    // Ball collision with players from team A
    ...
    // Ball collision with players from team B
    ...
    // Ball collision with pitch boundaries
    if (ball.getPosition().y <= 10 || ball.getPosition().y >= window.getSize().y - 10) {
        ball.reverseY();
    }
    
    // Ball collision with side boundaries (throw-in lines)
...
void Game::checkGoal() {
    ...
    // Goal area: center portion of the goal line
    float goalStartY = pitchHeight / 2 - 50;
    float goalEndY = pitchHeight / 2 + 50;
    
    // Goal for team B (ball passes left goal line)
    ...
    // Goal for team A (ball passes right goal line)
...
void Game::render() {
    window.clear(sf::Color(34, 139, 34));  // Green pitch color
