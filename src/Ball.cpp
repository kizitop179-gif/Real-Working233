...
void Ball::reverseX() { velocity.x = -velocity.x * 0.8f; }
void Ball::reverseY() { velocity.y = -velocity.y * 0.8f; }
void Ball::reset(float x, float y) {
    shape.setPosition(x - shape.getRadius(), y - shape.getRadius());
    velocity = sf::Vector2f(0, 0);
}
void Ball::setVelocity(float vx, float vy) { velocity.x = vx; velocity.y = vy; }
